...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                self.ant = insect
            elif self.ant.is_container and insect.is_container:
                assert self.ant is None, 'Two ants in {0}'.format(self)
            elif self.ant.is_container and (not self.ant.contained_ant):
                self.ant.contained_ant = insect
            elif insect.is_container and (not insect.contained_ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while (nearest_place is not hive):
            if nearest_place.bees and (self.min_range <= range <= self.max_range):
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        ...
        if not other.is_container:
            return True
        elif other.is_container and not other.contained_ant:
            return True
        else:
            return False
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while location:
                if location.ant:
                    ant = location.ant
                    if ant not in self.is_buffed:
                        ant.damage = 2 * ant.damage
                        self.is_buffed.append(ant)
                    if ant.is_container and ant.contained_ant and ant.contained_ant not in self.is_buffed:
                        ant.contained_ant.damage = 2 * ant.contained_ant.damage
                        self.is_buffed.append(ant.contained_ant)
